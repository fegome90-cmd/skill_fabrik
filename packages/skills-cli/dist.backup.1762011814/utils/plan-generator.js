 ...
 */
export async function generatePlanMarkdown(plan, outputPath) {
...
${plan.phases
        .map((phase, idx) => `### ${idx + 1}. ${phase.name}
...
${plan.risks.length > 0
        ? plan.risks
            .map((risk, idx) => `### ${idx + 1}. ${risk.description}
 ...
 */
export async function generateContextMarkdown(taskName, plan, outputPath) {
    const relevantFiles = [];
    const dependencies = [];
    const constraints = [];
 ...
 */
export async function generateTasksMarkdown(taskName, plan, outputPath) {
    const todoItems = [];
 ...
 */
export async function createPlanFromTaskV2(task, cwd) {
    const planId = generatePlanId();
    const now = new Date().toISOString();
