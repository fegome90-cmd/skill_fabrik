...
import { createPlanFromTask, createPlanFromTaskV2, generatePlanMarkdown, generateContextMarkdown, generateTasksMarkdown, } from '../utils/plan-generator.js';
// MemTech snapshot (optional - will fail gracefully if not available)
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function createPlanSnapshotFallback(data) {
    try {
        ...
        try {
            ...
            if (options.approve) {
                if (plan.status !== 'APPROVED') {
                    ...
                    const targetStatus = plan.status === 'DRAFT' ? 'APPROVED' : 'APPROVED';
            ...
            // Health check preventivo (no bloqueante)
            try {
                const mcp = await import('@skills-fabrik/mcp-adapters');
    ...
    planCmd
        ...
        try {
            ...
            for (const file of planFiles) {
                try {
                    const plan = (await readJson(path.join(plansBase, file)));
