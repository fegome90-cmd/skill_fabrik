...
export function hooksCommand(program) {
    program
        .command('hooks')
        ...
        try {
            ...
            for (const hookName of hooksToInstall) {
                try {
                    ...
                    results.errors.push(`${hookName}: ${error instanceof Error ? error.message : String(error)}`);
...
async function installUserPromptSubmitHook(hooksDir, _config) {
...
async function installStopHook(hooksDir, _config) {
    const hookScript = `#!/usr/bin/env node
/**
 * Stop Hook
