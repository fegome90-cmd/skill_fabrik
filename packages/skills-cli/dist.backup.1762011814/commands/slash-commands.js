...
async function loadSlashCommandSystem() {
    if (!slashCommandSystem) {
        try {
            ...
            throw new Error('Slash commands system not available. Please install @skills-fabrik/slash-commands package.');
        }
    }
    return slashCommandSystem;
}
export function slashCommandCommand(program) {
    program
        .command('/')
        ...
        .action(async (commandName, options, commandObj) => {
        try {
            ...
            const result = await executeSlashCommand(system, parsedCommand, commandDefinition);
...
async function executeSlashCommand(system, parsedCommand, commandDefinition) {
...
function getHandlerClass(system, handlerName) {
...
async function showCommandHelp(system, commandName, options) {
    const registry = system.SlashCommandRegistryManager.getInstance();
...
async function listAvailableCommands(registry, categoryFilter) {
...
async function showRegistryStats(registry) {
