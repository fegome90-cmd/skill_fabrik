...
import { promptSelect, promptConfirm, withSpinner } from '../../utils/progress.js';
const nav = new NavigationCore();
// Professional project navigation dashboard
export async function navDashboard() {
    console.clear();
    // Show welcome banner
    console.log(createBox('🧭 Skills Fabric Project Navigation Hub', 'Navigation System v1.0'));
    console.log('');
    while (true) {
        try {
...
export async function navGoto(argv) {
    const view = argv[4];
    if (!view) {
...
export async function navExplore() {
    ...
    console.log(format.bullet('Health Status:'), status.health === 'healthy' ? colors.success(status.health) :
...
export async function navCLI(argv) {
...
function showProjectStatus(status) {
    ...
    console.log(createBox(`Project: ${status.name}`, 'Current Status'));
    console.log('');
...
function showNavigationMenu() {
    ...
    return promptSelect('Select navigation action:', actions.map(action => `${action.shortcut ? `[${action.shortcut}] ` : ''}${action.title}`), 0).then(title => {
        const cleanTitle = title.replace(/^\[.\]\s+/, '');
        return actions.find(a => a.title === cleanTitle)?.id || null;
    });
}
async function executeNavigationAction(actionId, status) {
    switch (actionId) {
        case 'status':
            await navStatus();
            break;
