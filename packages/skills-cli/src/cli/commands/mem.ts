...
interface MemoryAction {
  ...
  description: string;
  icon: string;
  category: 'system' | 'configuration' | 'diagnostics' | 'maintenance';
  dangerous?: boolean;
}

// Memory system dashboard with interactive menu
export async function memDashboard(): Promise<void> {
  console.clear();

  // Show welcome banner
  console.log(createBox(
    '🧠 Skills Fabric Memory Management Dashboard',
    'Memory System v1.0'
  ));
...
export async function memSetup(): Promise<void> {
...
export async function memStatus(): Promise<void> {
...
export async function memTest(): Promise<void> {
...
export async function memSwitch(argv: string[]): Promise<void> {
  ...
  const backendOptions = availableBackends.map(backend =>
    `${getBackendIcon(backend)} ${backend} - ${getBackendDescription(backend)}`
  );

  const selectedBackendIndex = await promptSelect(
...
export async function memCLI(argv: string[]): Promise<void> {
...
function showMemoryStatus(status: MemoryStatus): void {
  ...
  if (status.storage.maxSize > 0) {
    ...
    console.log(format.bullet('Usage:'),
      percentage < 50 ? colors.success(`${percentage}%`) :
...
function showActionMenu(): Promise<string | null> {
  ...
  return promptSelect(
    ...
    0
  ).then(title => actions.find(a => a.title === title)?.id || null);
}

async function executeAction(actionId: string, status: MemoryStatus): Promise<void> {
  switch (actionId) {
    case 'status':
      await memStatus();
      break;
    ...
    case 'cleanup':
      const confirmed = await promptConfirm(
        'This will remove all stored memory data. Are you sure?',
        false
      );
      if (confirmed) {
        await clearMemory();
...
function getBackendDescription(backend: string): string {
  ...
  return descriptions[backend as keyof typeof descriptions] || 'Unknown storage backend';
}
...
function getBackendIcon(backend: string): string {
  ...
  return icons[backend as keyof typeof icons] || '❓';
}
...
function showConfigurationInfo(): void {
