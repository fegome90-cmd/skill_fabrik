...
let planCheckModule: {
  checkApprovedPlan: (
    cwd: string
  ) => Promise<{ hasPlan: boolean; plan?: unknown; taskName?: string }>;
} | null = null;
...
async function runPreHooks(input: PreHookInput): Promise<PreHookOutput> {
  ...
  const phase: PreHookOutput['phase'] = /clarify|aclarar/i.test(text)
    ? 'Clarify'
    : /layout|estructura/i.test(text)
      ? 'Layout'
      : /operar|ejecutar/i.test(text)
        ? 'Operate'
        : /observa|eval/i.test(text)
          ? 'Observe'
          : /reflex/i.test(text)
            ? 'Reflect'
            : 'Clarify';

  const tags = Array.from(text.matchAll(/\[(K|C|U|EVIDENCIA|PROPUESTA)\]/gi)).map(m =>
...
type PostHookInput = {
  prompt: string;
  signals: { tags?: string[]; templateComponents?: string[] };
};
type PostHookOutput = {
  prompt: string;
  meta: { score4D: number; tagsOut: string[]; summary: string };
};
    ...
    if (existsSync(packagesPath)) {
      ...
      for (const pkg of packages) {
        if (pkg.isDirectory()) {
          ...
          if (
            pkg.name.includes('api') ||
            pkg.name.includes('backend') ||
      ...
      structure.detectedPaths.packages = packages
  ...
  try {
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
      ...
      if (results.length === 0) {
        ...
        if (
          PARALLEL_CONFIG.enableParallel &&
          pathPatterns.length > PARALLEL_CONFIG.useWorkersThreshold / 100
        ) {
          try {
...
async function findRealFilesParallel(
  ...
  const searchPromises = pathPatterns
    .slice(0, PARALLEL_CONFIG.maxConcurrency)
    .map(async pattern => {
      try {
    ...
    try {
      ...
      if (pattern.includes('packages/**') || pattern.includes('**/memtech/**')) {
        if (projectStructure.detectedPaths.memtech) {
          ...
          searchPaths.push(
            ...projectStructure.detectedPaths.packages.map(p => resolve(cwd, p, 'src'))
          );
        }
      } else if (pattern.includes('**/repository/**')) {
        if (projectStructure.detectedPaths.backend) {
          searchPaths.push(
            ...projectStructure.detectedPaths.backend.map(p => resolve(cwd, p, 'src'))
          );
        }
        ...
        if (projectStructure.detectedPaths.frontend) {
          searchPaths.push(
            ...projectStructure.detectedPaths.frontend.map(p => resolve(cwd, p, 'src'))
          );
        }
...
const DEFAULT_PHASES = ['Clarify', 'Layout', 'Operate', 'Observe', 'Reflect'];

function resolvePhases(planContext?: PlanContext): string[] {
  if (planContext?.phases && planContext.phases.length > 0) {
    return planContext.phases.map(phase =>
      typeof phase === 'string' ? phase : phase.name || 'Fase'
    );
  }
...
function generateTemplateStructure(
  ...
  const phases = resolvePhases(planContext);
  const planSlug = slugify(description || skillId);
  const planTitle = titleCase(description || planSlug);
  const complexityLevel = options.complexity || 'high';
  const complexityConfig = getComplexityConfig(complexityLevel);
  const targetCoverage =
    typeof complexityConfig.target_coverage === 'number'
      ? complexityConfig.target_coverage
      : Math.round((complexityConfig.coverage || 0.85) * 100);
  const planReference = planContext?.planId
    ? `${planContext.planId}${planContext.taskName ? ` (${planContext.taskName})` : ''}`
    : 'Sin plan aprobado (usar planning gate)';
  const phaseBullets = phases
    .slice(0, 6)
    .map((phase, index) => `- **Fase ${index + 1} – ${phase}:** objetivos, dependencias y entregables`)
    .join('\n');
  const phaseArray = phases.map(p => `"${p}"`).join(', ');
  const roadmap = phases
    .map(
      (phase, index) => `### ${phase.toUpperCase()} · Semana ${index + 1}
- Objetivo principal:
- Entregables:
...
`
  ...
  const miniTaskTemplate = `#### Mini-Task T1.1: Definir acción crítica
\`\`\`yaml
...
`;
  ...
  const metricsTable = `| Métrica | Before | Target | After | Verificación |
|---------|--------|--------|-------|--------------|
| Coverage | 0% | ${targetCoverage}% | TBD | \`pnpm test -- --coverage\` |
| Latency | 200ms | 80ms | TBD | \`node scripts/test-latency.mjs\` |
| Incidentes | 5 | 0 | TBD | \`scripts/alerts-report.mjs\` |
`;

  const boundaryMarkers = `- **BM1**: Frontmatter YAML completo (id/version/fechas/coverage)
...
- **BM8**: Tests y validaciones obligatorias`;
  ...
  const templateChecklist = `- C1: CSE completo
- C2: TAGs cobertura >= 6
...
- C8: Evidencia vs Propuesta separadas`;
  ...
  return `---
meta:
  id: "startkit-${planSlug}"
  version: "1.0.0"
  created_at: "${today}"
  base: "${skillId}"
  mode: "implementation"
  anti_drift: true
  architecture: "${skillId.replace(/_/g, '-')}"
  dependencies: [${phaseArray}]
  target_coverage: ${targetCoverage}
  estimated_duration: "${complexityConfig.duration}"
  complexity: "${complexityLevel}"
  innovation_level: "${complexityConfig.innovation_level}"
...
- **Skill activado**: ${skillId}
- **Plan activo**: ${planReference}
- **Fases vigentes**:
${phaseBullets}
...
export async function buildOptimizedPromptV2(
  ...
  for (const skillId of skillIds) {
    ...
    const relevantKeywords = keywords
      .slice(0, 3)
      .filter(kw => !bestIntent.toLowerCase().includes(kw.toLowerCase()));
    const promptBase =
      relevantKeywords.length > 0 ? `${relevantKeywords.join(', ')}: ${bestIntent}` : bestIntent;
    ...
    if (skillIds.length === 1 || skillIds.indexOf(skillId) === 0) {
      ...
      if (planContext && options.includePlanContext) {
        optimizedPrompt += `\n\n📋 Plan activo: ${planContext.planId || 'Unknown Plan'} (${planContext.taskName || 'Unknown Task'})`;
        if (planContext.phases && planContext.phases.length > 0) {
          optimizedPrompt += `\nFases del plan: ${planContext.phases.map((p: any) => p.name || p).join(', ')}`;
        }
      }
  ...
  const result: OptimizedPrompt = {
    ...
    signals: {
      ...
      intent: Array.from(new Set(allIntents)),
      paths: Array.from(new Set(allPaths)),
      content: Array.from(new Set(allContent)),
      tags: options.includeTags ? Array.from(new Set(allTags)) : undefined,
      templateComponents: options.includeTemplate
        ? (['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8'] as string[])
        : undefined,
    },
  ...
  const post = runPostHooks({
    prompt: result.prompt,
    signals: { tags: result.signals.tags, templateComponents: result.signals.templateComponents },
  });
...
export { metricsCollector, projectIndexManager, workerThreadManager, PARALLEL_CONFIG };
