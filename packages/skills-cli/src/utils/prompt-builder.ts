...
// Tipos locales (compatibles con router)
...
export interface PromptBuilderOptions {
  skillId?: string; // Skill específico a activar
  description: string; // Descripción del usuario
  includeFiles?: boolean; // Incluir sugerencias de archivos
  includeContent?: boolean; // Incluir snippets de contenido
...
export interface OptimizedPrompt {
  prompt: string; // Prompt completo optimizado
  expectedScore: number; // Score esperado (0.0-1.0)
  signals: {
    keywords: string[]; // Keywords usadas
    intent: string[]; // Intent patterns usados
    paths: string[]; // Paths sugeridos
    content: string[]; // Content patterns usados
    tags?: string[]; // TAGs generados (v2)
    templateComponents?: string[]; // Componentes Template v1.1.0 (v2)
  };
  skillActivation: {
    skillId: string;
    score: number;
  ...
  templateScore?: number; // Score de Template v1.1.0 (v2)
  tagsCoverage?: number; // Coverage de TAGs (v2)
  planContext?: {
    planId?: string;
    taskName?: string;
    phases?: Array<{ name: string }>;
  }; // Contexto de plan activo (v2)
}
...
export async function buildOptimizedPrompt(
  ...
  const tempScore = calculateExpectedScore(
    optimizedPrompt,
    suggestedFiles,
    suggestedContent,
    rule
