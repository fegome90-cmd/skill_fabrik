...
export function extractPhaseNumber(phase: string): number {
...
export async function generatePlanStart(phase: string): Promise<string> {
...
export async function generatePresprint(
  phase: string,
...
export async function validateQualityGate(gate: string, _phase: number): Promise<boolean> {
...
export async function validatePreviousPhase(phaseNum: number): Promise<void> {
  if (phaseNum === 0) {
    return; // F0 has no previous phase
  }

  const previousPhase = `F${phaseNum - 1}`;
  const previousPresprint = path.join(
    process.cwd(),
    'cloop',
    'phases',
    `${previousPhase}-presprint.md`
  );

  if (!(await pathExists(previousPresprint))) {
    throw new Error(
      `Previous phase ${previousPhase} not completed. ` +
