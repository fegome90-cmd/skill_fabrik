...
/**
 * Database Security Validation Interfaces
 * Applied with database-verification skill - Template v1.1.0
 */
...
export class StateManager {
  ...
  private mutationLimits: MutationLimit[];
  ...
  async loadNavigationState(): Promise<any> {
    const filePath = path.join(this.stateDir, 'navigation.json');
  ...
  async appendMetrics(metric: any): Promise<void> {
  ...
  async saveLastRun(run: any): Promise<void> {
  ...
  private async readJsonFile(filePath: string, defaultValue: any = {}): Promise<any> {
    try {
   ...
   */
  private async validateOperation(operation: string, data: any): Promise<{
    valid: boolean;
    message?: string;
    level?: 'warn' | 'error' | 'block';
  }> {
  ...
  private checkRateLimit(operation: string): {
    valid: boolean;
    message?: string;
    level?: 'warn' | 'error' | 'block';
  } {
    ...
    const relevantTimestamps = timestamps.filter(
      timestamp => now - timestamp < 60000 // 1 minute window
    );
  ...
  private validateNoMassiveDeletion(data: any): boolean {
  ...
  private validateNoMassiveUpdate(data: any): boolean {
      ...
      const checkCircular = (obj: any): boolean => {
        if (obj && typeof obj === 'object') {
          ...
          for (const value of Object.values(obj)) {
            if (!checkCircular(value)) return false;
          }
        }
        return true;
      };
  ...
  private validateSafeStructure(data: any): boolean {
  ...
  showSecurityDashboard(): void {
    ...
    console.log(
      createBox(
        format.header('🔒 Database Security Dashboard') + '\n\n' +
        ...
        this.validationRules.map(rule =>
          format.bullet(rule.name, rule.level === 'block' ? colors.error('BLOCK') :
                       rule.level === 'error' ? colors.warning('ERROR') : colors.info('WARN'))
        ).join('') + '\n' +

        format.section('Rate Limits') +
        this.mutationLimits.map(limit =>
          format.bullet(`${limit.operation}`, colors.text(`${limit.maxRecords}/min`))
        ).join('') + '\n' +
