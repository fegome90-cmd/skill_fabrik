# Dashboard de KPIs - Skills Fabric

**Estado Holístico**: 🔴 **CRITICAL**

**Período**: 2025-10-27 - 2025-11-03  
**Total de Eventos**: 1

---

## 📊 Métricas en Pares

### ⚡ Velocidad (Activación y Eficiencia)

| Métrica | Valor | Umbral | Estado |
|---------|-------|--------|--------|
| **Tasa de Activación de Skills** | 0.00 skills/op | ≥ 0.5 | ⚠️ |
| **Tokens por Operación** | 0 tokens | ≤ 25,000 | ✅ |
| **Latencia Promedio** | 69 ms | < 10,000 ms | ✅ |
| **Divulgación Progresiva** | 0.0 recursos/op | ≤ 2 | ✅ |

### 🎯 Calidad (Adherencia y Prevención)

| Métrica | Valor | Umbral | Estado |
|---------|-------|--------|--------|
| **Tasa de Adherencia** | 0.0% | ≥ 80% | ❌ |
| **Zero Errors Rate** | 0.0% | ≥ 95% | ❌ |
| **Latencia de Corrección** | 0.0s | < 5 min | ✅ |
| **Efectividad de Guardrails** | 100.0% | ≥ 90% | ✅ |

---

## 🔍 Interpretación Holística

### 🔴 Estado: Crítico

**Desequilibrio crítico detectado**. El sistema no está cumpliendo objetivos de calidad o velocidad.

**Áreas Críticas**:


- ❌ Adherencia a guías muy baja

- ❌ Muchos errores residuales


**Acción Urgente**: Revisar configuración, triggers de skills y guardrails inmediatamente.

---

## 📈 Activaciones por Skill

*No hay activaciones registradas*

---

## 🎯 Próximos Pasos

- **Mejorar adherencia**: Revisar triggers de skills y ajustar thresholds
- **Reducir errores residuales**: Mejorar guardrails y auto-resolver

---

*Generado el 2025-11-03T23:31:20.925Z*
