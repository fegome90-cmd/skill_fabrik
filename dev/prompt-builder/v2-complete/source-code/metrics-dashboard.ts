...
class MetricsCollector {
  ...
  private checkThresholds(metric: PerformanceMetrics): void {
    ...
    if (metric.duration > this.thresholds.latency) {
      alerts.push(`⚠️ High latency: ${metric.duration.toFixed(2)}ms > ${this.thresholds.latency}ms`);
    }

    if (metric.memoryUsage > this.thresholds.memory) {
      alerts.push(`⚠️ High memory: ${metric.memoryUsage.toFixed(2)}MB > ${this.thresholds.memory}MB`);
    }
    ...
    if (metric.cacheHitRate < this.thresholds.cacheHit) {
      alerts.push(`⚠️ Low cache hit rate: ${metric.cacheHitRate.toFixed(1)}% < ${this.thresholds.cacheHit}%`);
    }
